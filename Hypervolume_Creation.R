...
# Script 2 of 7 authored by Steven N. Winter
...
negatives<- subset(dat, dat$status>0)
...
setwd("//idstorage.cnre.vt.edu/IDStorage1/Students/Steven_Winter/CWD/Raw_VA_CWD_Data/Presence_only_GeoPartitioning_Eval")
...
positives_reduced$block <- as.factor(blocks$occ.grp)
...
for(i in 1:3){
  ...
  #Harvest Location scale
  hv_train<- hypervolume_gaussian(training.data,
                                  name='training',
  ...
  #Home Range scale
  hv_buffer<- hypervolume_gaussian(training.buffer,
  ...
  #Harvest Location scale
  svm_hv_train<- hypervolume_svm(training.data, name='training') # Defaults with gamma and nu values of 0.5 and 0.1, respectively.
  
  #Home Range scale
  svm_hv_buffer<- hypervolume_svm(training.buffer, name='training.buffer',)
  
  
  #KDE
  
  # Project binary KDE maps with 0.95 threshold
  hyper_map <- hypervolume_project(hv_train, rasters = env.layers, type = "inclusion", fast.or.accurate="accurate", verbose = T)
  hyper_buffer_map <- hypervolume_project(hv_buffer, rasters = env.layers, type = "inclusion", fast.or.accurate="accurate", verbose = T)
  ...
  output_buffer_filename<- paste0('./KDE_Maps/Buffer_Binary',n1,'_and_',n2, "_Calibration") #Home Range
  ...
  hyper_cont_map <- hypervolume_project(hv_train, rasters = env.layers, type = "probability", verbose = T)
  hyper_buffer_cont_map <- hypervolume_project(hv_buffer, rasters = env.layers, type = "probability", verbose = T)
  ...
  output_filename<- paste0('./KDE_Maps/Standardized_Point_Continuous',n1,'_and_',n2, "_Calibration")
  output_buffer_filename<- paste0('./KDE_Maps/Standardized_Buffer_Continuous',n1,'_and_',n2, "_Calibration")
  ...
  svm_hyper_map <- hypervolume_project(svm_hv_train, rasters = env.layers, type = "inclusion", fast.or.accurate="accurate", verbose = T)
  svm_hyper_buffer_map <- hypervolume_project(svm_hv_buffer, rasters = env.layers, type = "inclusion", fast.or.accurate="accurate", verbose = T)
  
  # Create filenames and write SVM rasters for evaluations
  output_filename<- paste0('./SVM_Maps/Point_SVM',n1,'_and_',n2, "_Calibration")
  output_buffer_filename<- paste0('./SVM_Maps/Buffer_SVM',n1,'_and_',n2, "_Calibration")
...
for(i in 1:3){
  ...
  # Project binary KDE maps with 0.95 threshold
  hyper_map <- hypervolume_project(hv_train, rasters = env.layers, type = "inclusion", 
                                   fast.or.accurate="accurate", verbose = T)
  hyper_buffer_map <- hypervolume_project(hv_buffer, rasters = env.layers, type = "inclusion", 
                                          fast.or.accurate="accurate", verbose = T)
  # Create filenames and write KDE rasters for evaluations
  output_filename<- paste0('./KDE_Maps/Point_Binary_',n1,'_and_',n2, "_Calibration") #Harvest Location
  output_buffer_filename<- paste0('./KDE_Maps/Buffer_Binary',n1,'_and_',n2, "_Calibration") #Home Range
  ...
  # Project binary SVM maps with 0.95 threshold
  svm_hyper_map <- hypervolume_project(svm_hv_train, rasters = env.layers, type = "inclusion", 
                                       fast.or.accurate="accurate", verbose = T)
  svm_hyper_buffer_map <- hypervolume_project(svm_hv_buffer, rasters = env.layers, type = "inclusion", 
                                              fast.or.accurate="accurate", verbose = T)
